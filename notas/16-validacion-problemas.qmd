...
intentar recuperar a clientes abandonadores. Una fracción los clientes que abandonaron entre el tiempo 0.3 y 1.5 recibieron una llamada de servicio a cliente. Esto está registrado en la base de datos.
...
simular_clientes <- function(n,...){
    ...
    # si se hizo un contacto
    llamada <- ifelse(tiempo_cliente > 0.3 & tiempo_cliente < 1.5,
                      rbinom(1, 1, 0.98), 0)
...
datos_mod <- clientes_1 |> 
  left_join(num_visitas_hoy |> select(id_cliente, num_visitas))
```
...
entrena <- filter(datos_mod, u < 0.8)
valida <- filter(datos_mod, u >= 0.8)
```
